...
def build_model(
    ...
    # initialize all tracked visited points to False at time 0
    for point_variables in visited.values():
        solver.add(point_variables[0] == False)

    # transitions for (0 <= time <= max_sum)
...
def z3_int(model: ModelRef, expression) -> int:
    # checks Z3 int expression and returns integer
...
def print_trace(
    ...
    model_data: dict,
    final_time: int,
    trace_title: str = "Counterexample trace:",
) -> None:
    row = model_data["row"]
 ...
 # verifies that no wrong complete path exists
def check_always_right_path(
    ...
    # checks if there exists a complete path that ends in an invalid column
    bad_final_state = And(
...
# check if a specific sum is valid
def check_specific_sum(elements: Sequence[int], target_sum: int) -> bool:
    validate_elements(elements)
    ...
    if target_sum in valid_sums:
        print(f"YES: {target_sum} is a valid subset sum.")
        return True

    print(f"NO: {target_sum} is not a valid subset sum.")
    return False

# shows correct path/s
def show_correct_path(
    ...
    if target_sum is not None:
        if target_sum not in valid_sums:
            print(
                f"No correct path exists for {target_sum}, "
                "because it is not a valid subset sum."
            )
            return False

        print(f"Searching for a correct path ending at column {target_sum}.")

        correct_final_state = And(
            row[final_time] == max_sum,
            column[final_time] == target_sum,
        )

    else:
        print("Searching for any correct path.")
    ...
    solver.add(correct_final_state)
    ...
    if result == sat:
        ...
        model = solver.model()
        print_trace(model, model_data, final_time, "Correct path trace:")
...
def main() -> None:
    ...
    # each 2 elements will convey a point of (3,1) - to see if the path has passed through it
    parser.add_argument(
        "--track",
        metavar="ROW,COLUMN",
    ...
    parser.add_argument(
        "--check-sum",
        type=int,
        help="Check whether a specific target sum is a valid subset sum.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Show a correct path. "
            "Use without value for any valid path, or with a number for a specific final sum."
        ),
    )
    ...
    try:
        if args.check_sum is not None:
            check_specific_sum(
                elements=args.elements,
                target_sum=args.check_sum,
            )

        elif args.show_correct_path is not None:
            if args.show_correct_path == -1:
                show_correct_path(
                    elements=args.elements,
                ...
                show_correct_path(
                    elements=args.elements,
                    target_sum=args.show_correct_path,
                    tracked_points=args.track,
                )
        ...
        # 2. Stop timer and extract peak memory usage for Windows
        end_time = time.perf_counter()
        process = psutil.Process(os.getpid())
        peak_memory_kb = process.memory_info().peak_wset / 1024
